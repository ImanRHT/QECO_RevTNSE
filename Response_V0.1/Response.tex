...
\textcolor{purple}{\textit{``(a) Lots of the studies focus on the research on resource optimisation of MEC by DRL, it is expected to survey the state-of-the-art within 3 years, and summarise it as an individual section, departing from the Section of Introduction. (b) Besides, the existing related work only has one paragraph without any discussion about the pros/cons of the previous work.''}} \newline
...
\begin{enumerate}
	...
	\item \underline{Reviewer's Comment}: 
	\textit{``\textcolor{purple}{The authors should provide a comparative table to highlight their contributions. Specially, the authors should compare their method with other references on computation task offloading cited in the paper, including aspects such as scenarios, methods, advances, drawbacks, and so on.''}} \newline

	\underline{Authors' Reply}: To address the reviewer's comment, we have added a table comparing the characteristics of related works in the related works subsection, as below:
	\begin{table*}[htbp]\textcolor{blue}{
			\renewcommand{\arraystretch}{0.6}
			\captionsetup{name=TABLE}
			\caption{Comparison of Related Works Characteristic\vspace{4mm}}
	...
	\item \underline{Reviewer's Comment}: 
	\textit{``\textcolor{purple}{(a) What inputs and outputs of Algorithm 2? (b) They should be added to make readers clearly understand the results.''}} \\\newline
...
\vspace{10mm}
	\item \underline{Reviewer's Comment}: 
	\textit{``\textcolor{purple}{The authors should open their experiments in github, so that other researchers can compare their work with other ones.''}} \newline
...
\newpage
	...
	\item \underline{Reviewer's Comment}: 
	\textit{``\textcolor{purple}{The following references closely related to the current work should be discussed or compared with the current work for completeness.''}} \newline
		\begin{my}{1cm}{1cm}
		\rev{
			{\small
				\noindent\hspace{-0.5mm}\cite{wu2023multi} G. Wu, Z. Xu, H. Zhang, S. Shen, and S. Yu,  ``Multi-agent DRL for joint completion delay and energy consumption with queuing theory in MEC-based IIoT,'' in \textit{Journal of Parallel and Distributed Computing}, vol. 176, pp. 80–94, Jun. 2023.\\[6pt]
				...
				\cite{wu2024privacy} G. Wu, X. Chen, Z. Gao, H. Zhang, S. Yu, and S. Shen, ``Privacy-preserving offloading scheme in multi-access mobile edge computing based on MADRL,'' \textit{Journal of Parallel and Distributed Computing}, vol. 10, no. 1, pp. 451--460, Mar. 2022.\\[6pt]
				\cite{wu2024combining} G. Wu, X. Chen, Y. Shen, Z. Xu, H. Zhang, S. Shen, and S. Yu,``Combining Lyapunov optimization with Actor-Critic networks for privacy-aware IIoT computation offloading,'' in \textit{IEEE Internet of Things Journal}, 2024, Early Access\\[10pt]}\\}
	\end{my}
