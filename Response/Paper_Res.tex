...
\setcounter{equation}{16}
	...
	\rev{
		{\ \color{black}
%\begin{alignat}{1}
%	E_{i,j}^{\text{I}}(t) = \textcolor{blue}{\sum_{\mathcal{J}} y_{i,j}(t)} D_{i,j}^{\text{E}}(t) p_i^{\text{I}} \tau, %= {\lambda_{i,j}^{\text{E}}(t) \rho_i(t) \over \mathcal{B}_j(t) f_j^{\text{E}}} p_i^{\text{I}},
%	\label{16}
%\end{alignat}
\begin{alignat}{1}
...
(b) Since DRL methods typically require data in the form of experiences or interaction sequences with an environment, there are no readily available and accessible real datasets that satisfy the core concept of training neural networks (i.e., state, action, reward, and next state) in DRL-based algorithm. We simulated an MEC environment aims to capture the interaction between mobile devices and edge servers, allowing the DRL agent to gain experiences and help generate the necessary data in the desired format for training DRL agents. based on Queuing Theory, Our simulated model provide key system performance metrics, including task delay, energy consumption, and etc, which enable the agent to evaluate each state-action value and define the mobile device's QoE. In this environment, we incorporate multiple mobile devices and edge servers to empower the DRL agents to learn and gather the necessary data for effective training. To train the MDs' neural networks, we adopt a scenario comprising 1000 episodes. Each episode contains 100 time slots, each of length 0.1 second. In each time slot, each mobile device generates computation tasks according to a task generation probability. The offloading decision for newly arrived tasks is made by the DRL agent, considering its state that includes the newly arrived task size, the queues information, the MD’s energy level, and the workload history at the ENs. The agents evaluate thair action in that sate by the mobile device's QoE that reflect the key performance metrics based on user preference. 
