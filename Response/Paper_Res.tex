...
\underline{Authors' Reply}: To address the reviewer’s comment, we further specify the contribution and highlight the role of the LSTM network and its impact on the convergence and performance of the QECO algorithm. We have revised the Subsection on motivations and contributions in Section I, as follows:
...
\item \textit{Performance Evaluation:} We conducted comprehensive experiments to evaluate QECO's performance under various computational workloads, as well as its training convergence with different techniques and hyperparameters. The results demonstrate that our algorithm effectively utilizes the processing capabilities of MDs and ENs, addressing the dynamic workloads at ENs. It converges more quickly compared to DQN and D3QN methods, while providing a substantial improvement of at least 34.8\% in average QoE. This advantage is achieved through prioritizing MD's QoE preferences, resulting in improvements of at least 12.2\%, 11.7\%, and 10.8\% in the average number of completed tasks, task delay, and energy consumption, respectively, when compared to the distributed and collective DRL-based offloading algorithm (DCDRL) \cite{qiu2020distributed} and several benchmark methods.
	
	
\end{itemize}}}
\end{my}

We have also extended the performance evaluation by comparing vanilla DQN and D3QN to further specify the impacts of the LSTM network, Dueling DQN, and Double DQN. The following chart and discussion have been added to the performance comparison subsection of Section V, as bellow: 
...
\underline{Authors' Reply}: (a) The task size is selected randomly from a discrete set to avoid state space complexity. To address the reviewer’s comment, we have added a mention of the random task size selection to the second paragraph of Section II, as follows:
...
\underline{Authors' Reply}: (a) To address the reviewer's comments, we extended the evaluation of the QECO algorithm by comparing it with DCDRL \cite{qiu2020distributed}, a distributed optimization algorithm for delay-sensitive tasks based on the actor-critic framework (the foundation for many state-of-the-art DRL algorithms, including DDPG and PPO). We have added this comparison in the Simulation Setup Subsection of Section V as follows: \newline 
...
Furthermore, to enhance the evaluation of our work, we have extended the performance analysis by comparing QECO with vanilla DQN and D3QN. This comparison illustrates the impact of Dueling DQN, Double DQN, and LSTM network techniques, by the following chart and discussion:
...
\newpage
	...
	\underline{Authors' Reply}: To address the reviewer's comments, we have revised the QoE Function Subsection in Section III, as follows: 
