...
Sun \textit{et al.} in \cite{sun2024hierarchical} explored both computation offloading and service caching problems in MEC. They formulated an optimization problem that aims to minimize the long-term average service delay. They then proposed a hierarchical DRL framework, which effectively handles both problems under heterogeneous resources. Due to the dynamic nature of MEC systems, Li \textit{et al.} in \cite{li2022integrated} proposed an integrated optimization learning framework called DROO, which leverages past experience and model informa- tion to provide fast and robust real-time offloading control.
In \cite{li2020joint} the authors proposed an offloading strategy through the DRL algorithm to design the system, where a DQN is used to automatically learn offloading decisions, optimize system performance, and train a neural network (NN) to predict offloading actions. 
To reduce the total delay and waiting time of mobile vehicle tasks, Tang  \textit{et al.} in \cite{tang2022double}  established a dynamic offloading model of mul- tiple mobile vehicles, dividing its tasks into sequential subtasks, and making the offloading decision more refined.
