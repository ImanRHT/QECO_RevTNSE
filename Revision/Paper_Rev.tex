...
In contrast to these SOO studies, recent efforts have focused on conflicting multiple-objective optimization (CMOO) in MEC,  where RL are mainstream optimization techniques. CMOO often involves conflicting objectives, requiring careful balancing in decision-making. For example, task offloading must navigate trade-offs between delay and energy consumption, as well as balance energy usage with the number of completed tasks. 
