...
For energy-constrained MEC environments, reducing energy consumption becomes a key objective, motivating the creation of energy-efficient approaches. 
...
Some research addresed deadline-constraint tasks in energy-constrained MEC and proposed DRL-based approaches to strik a trade-off between task completion/drop ratio and energy consumption.
...
To address time-sensitive applications in energy-constrained MEC systems, striking a trade-off between delay and energy is the primary optimization objective, driving the development of delay-energy-optimal approaches in MEC.
...
To optimize delay and energy consumption, Wu \textit{et al.} in \cite{wu2023multi} investigated the computation offloading problem in a queuing-based MEC IIoT system. They modeled the problem as a POMDP and proposed a multi-agent proximal policy optimization (PPO)-based method to obtain the optimal offloading strategy in dynamic environments. 
 ...
 To address time-sensitive applications with prossecing deadline in energy-constrained MEC systems, achiving a balancing among delay, energy, and number of compeleted task is the optimization objective.
...
Huang \textit{et al.} in \cite{huang2019deep}, focused on a wireless-powered MEC and proposed a DRL-based approach, capable of attaining near-optimal decisions. This is achieved by selectively considering a compact subset of candidate actions in each iteration. 
 ...
 Althogh each of delay, energy consumption, and the number of completed tasks are critical factors in the design of MEC systems, but considering one of them as optimization objective \cite{li2022integrated}--\cite{dai2020edge}, which represents a significant research focus in MEC, can not reflect user satisfaction. 
 
 
 Some existing works primarily focus on simplified MEC networks, investigating single MEC network models. For instance, studies \cite{zhou2021deep}, \cite{liu2021learn} only examine single-server or single-user scenarios, while \cite{huang2019deep}--\cite{wu2023computation} do not account for delay-sensitive tasks or processing deadline. 
 
 Multi-objective studies \cite{chen2021drl}-\cite{gao2022large} have mostly investigated the trade-off between two or three of these factors but fail to properly address user demands and fulfill QoE requirements. QoE is a time-varying performance measure that reflects user satisfaction, where each user may have unique QoE requirements according to the MDs' individual preferences.
 ...
 In MEC environments, multiple objectives often compete, requiring a balance in decision-making. For instance, task offloading must carefully navigate trade-offs between delay and energy consumption, as well as balance energy usage against the number of completed tasks. 
 
 
 
 
 
 Considering delay and energy consumption as optimization objectives represents a significant research focus in MEC. 
 
 Most existing studies tend to optimize these two objectives independently, often neglecting the conflicts that arise between them, which can lead to skewed optimization results. 
 
 
 Research has increasingly addressed these multi-objective optimization challenges.
 ...
 \begin{table*}[htbp]\textcolor{blue}{
 		...
 		\scalebox{1}{%
 			\begin{tabular}{ lp{4cm}p{4.6cm}p{2cm}p{4.7cm}l} 
 				...
 				\cite{tang2022double}  & Computation offloading & Minimize total system delay & DDQN & Not consider system energy consumtion \\ \midrule
